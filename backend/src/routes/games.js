  ...
  try {
    ...
    switch (gameType) {
      case 'dice': {
        ...
        odds = betData?.payout || OddsCalculator.getPayoutForTarget ? OddsCalculator.getPayoutForTarget(target, 0.05) : (betData?.payout || 1);
        won = !!(gameResult && (gameResult.result === 'win' || gameResult.result === 'won'));
        break;
      }
      case 'crash': {
    ...
    // Apply global multiplier cap
    const MAX_MULTIPLIER = parseFloat(process.env.MAX_MULTIPLIER || '2.0');
    const cappedOdds = Math.min(odds, MAX_MULTIPLIER);
    const payout = won ? parseFloat(OddsCalculator.calculatePayout(amount, cappedOdds)) : 0;
    const profit = payout - amount;
    const payoutCapped = cappedOdds < odds;
    ...
    const betRecord = await Bet.create({
      ...
      houseEdge: 0.05,
      rtp: null,
      metadata: { gameResult, encryptedServerSeed: encryptedSeed, rigged, originalOdds: odds, cappedOdds, payoutCapped }
    }, { transaction: t });
