...
// Helper: create a pending manual transaction (authenticated)
router.post('/submit', auth, async (req, res) => {
  try {
    const user_id = req.user && (req.user.userId || req.user.id || req.user.sub);
    const { amount_paise, method, provider_txn_id, payer_vpa, payer_account } = req.body;
    if (!user_id || !amount_paise || !method || !provider_txn_id) {
      return res.status(400).json({ error: 'amount_paise, method and provider_txn_id required and user must be authenticated' });
    }
...
// Status endpoint (for polling/fallback) - user or admin
router.get('/status/:txId', auth, async (req, res) => {
  try {
    const txId = req.params.txId;
    const [rows] = await sequelize.query('SELECT id as tx_id, status, amount, currency, created_at, user_id FROM transactions WHERE id = $1 LIMIT 1', { bind: [txId] });
    if (!rows || !rows.length) return res.status(404).json({ error: 'Not found' });
    const tx = rows[0];
    const requester = req.user && (req.user.userId || req.user.id || req.user.sub);
    const isAdmin = req.user && (req.user.isAdmin || req.user.role === 'admin');
    if (!isAdmin && tx.user_id !== requester) return res.status(403).json({ error: 'Forbidden' });
