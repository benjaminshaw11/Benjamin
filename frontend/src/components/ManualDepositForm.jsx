...
export default function ManualDepositForm({ apiBase = '/api', jwt }) {
  ...
  async function submit(e) {
    ...
    try {
      const res = await fetch(`${apiBase}/manual-deposits/submit`, {
        ...
        body: JSON.stringify({ amount_paise: Math.round(parseFloat(amount)*100), method: 'upi', provider_txn_id: txnId })
      });
      const data = await res.json();
